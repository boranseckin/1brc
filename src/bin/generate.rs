...
fn main() {
    ...
    if args.len() < 3 {
        panic!("usage: cargo run --bin generate <count> <file>")
    }

    let entry_count = args[1].parse::<usize>().unwrap();
    ...
    for i in 1..=entry_count {
        if i % 10000 == 0 {
            ...
            println!("generated {} measurements in {:.3e} seconds", i, elapsed);
        }

        let (city, mean) = DATA.choose(&mut rng).unwrap();
        let dist = Normal::new(*mean, 10.0).unwrap();
        let value = dist.sample(&mut rng);
        writeln!(writer, "{city};{:.1}", value).unwrap();
    }
}
